...
od_drive_gear_retainer = 2*r_hub + 4;

/* [Clamp Gearing Design] */
module_clamp_gear = 1.2;
n_teeth_clamp_gear = 11; // [9, 10, 11, 12, 13]
...
module pure_vitamin_slide(as_clearance = false, center = ABOVE, show_for_design = false) {
    ...
    dz_screw = 0;
    dz_inner_bearing = dz_screw + od_drive_gear_retainer/2 - (d_filament/2 - clamping);
    ...
    module shape() {
        triangle_placement(r=0) {
            rotate([0, 0, 60]) {
                difference() {
                    hull() {
                        translate([r_hub+1, 0, 0]) block([1, y_spoke_base, h_hub], center=FRONT);
                    ...
                    //rotate([0, 0, 60]) translate([0, 0, h_hub/2]) retention_shaft_screws(as_clearance=true, cld=0.4);  // Pass through easily
                }
            }
        }
    }
...
module drive_gear(show_vitamins=true, include_bottom_gear=false) {
    ...
    h_hub_retainer = (od_clamp_gear - id_clamp_gear)/2;
    module additions() {        
        translate([0, 0, -dz_base]) can(d=od_drive_gear, h=h_base_drive_gear, center=BELOW);     
        translate([0, 0, -dz_base]) 
            can(d=id_hub_retainer, taper=od_hub_retainer, h=h_hub_retainer, center=ABOVE);
    }
    module removals() {
...
module retention_shaft_screws(
        as_clearance=true, 
        as_pads = false, 
        cld=0.4, 
        screw_length = 4, 
        pad_height=0) {    
    r_retention_screws = 9;
    if (as_pads) {
        rotate([0, 0, 60])  triangle_placement(r=r_retention_screws)  can(d=8, h=pad_height, center=ABOVE);
    } else {
...
module drive_gear_retainer(
    ...
    md =  2*r_hub + 2*clearance;     
     visualize(visualization_drive_gear_retainer) {   
        render(convexity=10) difference() {
                union() {
                    translate([0, 0, h_hub/2]) can(d=od_drive_gear_retainer, hollow=md, h = 3.75, center=ABOVE);        
                    translate([0, 0, h_hub/2+2+clearance]) can(d=od_drive_gear_retainer, hollow=id, h = 1.5 , center=ABOVE); 
                    translate([0, 0, h_hub/2]) can(d=od_drive_gear_retainer, hollow=md, h = h_hub, center=BELOW);    
                }
