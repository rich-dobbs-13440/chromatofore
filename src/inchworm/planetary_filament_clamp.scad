...
n_teeth_clamp_gear = 10; // [9, 10, 11, 12, 13]
...
clearance_for_d_cap = 1.25; // [0.5, 1.25]
...
visualization_clamp_gear = 
    visualize_info(
        "Clamp Gear", PART_3, gear_for_clamp, layout_from_mode(layout), show_parts); 

visualization_slide = 
    visualize_info(
        "Screw Slide", PART_4, slide_for_clamp, layout_from_mode(DESIGNING), show_parts); 

visualization_clamp_drive_gear = 
    visualize_info(
        "Clamp Drive Gear", PART_5, drive_gear_for_clamp, layout_from_mode(layout), show_parts); 
...
visualization_clamp_drive_shaft = 
    visualize_info(
        "Clamp Drive Shaft", PART_7, shaft_for_clamp, layout_from_mode(layout), show_parts);
...
clamp_gear_slide();
drive_gear_retainer(include_spacer=include_drive_gear_spacer);
...
module hub(show_vitamins=true) {
    visualize(visualization_clamp_hub) {
        render(convexity=10) difference() {
            ...
            retention_shaft_screws(as_clearance=true, cld=0.4);  // Pass through easily
...
module flange_screws(
        as_clearance=false, 
        include_head = false, 
        flange_screw_offset = flange_screw_offset) {
...
//module clamp_drive_gear_flange(center=ABOVE, h=h_clamp_drive_gear_flange) {
//    flange_screw_padding = 1;
//    s = 2 * flange_screw_offset + flange_screw_padding;
//    difference() {
//        block([s, s, h], center=center);
...
module clamp_drive_gear(show_vitamins=true, include_bottom_gear=true) {
    ...
    if (show_vitamins) {
        visualize_vitamins(visualization_clamp_drive_gear) {
...
module drive_gear_retainer(
    ...
    module blank() {
        ...
        translate([0, 0, h_shaft]) 
            can(d = d_cap, h = h_cap, center=BELOW);
    ...
    module spacer() {
        ...
        difference() {
            translate([0, 0, h_shaft]) can(d = od_spacer, center=BELOW, h = h_cap + 1);
    ...
    if (as_clearance) {
        ...
        translate([0, 0, dz]) {
            if (show_vitamins) {
                visualize_vitamins(visualization_clamp_drive_retainer) {
                    translate([0, 0, 0]) retention_shaft_screws(as_clearance=false, screw_length=l_shaft_screws); 
                }
            }
            visualize(visualization_clamp_drive_retainer) {
